//! Self-contained SHA-256 (FIPS 180-4) used for digests that pin bootstrap draws and the committed power table.
//!
//! Written in-crate so `portfolio-eval` has zero dependencies. It is verified in the tests against the NIST test vectors
//! and against `sha256sum` output for the committed fixtures, so a bug here cannot hide.
...
impl Sha256 {
    ...
    pub fn finalize(mut self) -> [u8; 32] {
        ...
        // `update` also bumps total_len, but the length was captured above.
        self.update(&pad);
...
/// One-shot SHA-256 as lower-case hex.
...
mod tests {
    ...
    #[test]
    fn nist_vector_million_a_and_streaming_equals_one_shot() {
        let data = vec![b'a'; 1_000_000];
        assert_eq!(sha256_hex(&data), "cdc76e5c9914fb9281a1c7e284d73e67f1809a48a497200e046d39ccc7112cd0");
        // Streaming in awkward chunk sizes must agree with the one-shot digest.
        let mut h = Sha256::new();
    ...
    #[test]
    fn padding_boundaries_55_56_63_64_65_bytes() {
        // Reference values from `sha256sum` of N ASCII 'a' bytes, computed independently of this implementation.
        let cases = [
