...
//!
//! REAL-DATA, ENV-GATED. These are the original golden tests of the rule crate, unchanged except for where the data
//! comes from: the ladder candles and the shadow signals are vendor-derived and are NOT in this (public) repository.
//! Set `REFRULES_LADDER_DIR` to the local `replication_ladder` folder to run them; without it each test prints
//! `SKIPPED` and passes. The always-on counterpart on the small synthetic fixtures is `golden_synthetic.rs`.
...
fn golden_inputs_are_the_recorded_files() {
    let Some(_) = real_or_skip("golden_inputs_are_the_recorded_files") else {
        return;
    };
    assert_eq!(sha256_of_real("ladder_candles.csv"), LADDER_SHA);
    assert_eq!(sha256_of_real("shadow_S1_monthend_signals.csv"), S1_SHA);
    assert_eq!(sha256_of_real("shadow_S3_daily_signals.csv"), S3_SHA);
}
...
fn golden_etf_every_month_end_explicit_mode() {
    let Some(_) = real_or_skip("golden_etf_every_month_end_explicit_mode") else {
        return;
    };
    let panel = etf_panel();
...
fn golden_etf_next_month_bar_mode() {
    let Some(_) = real_or_skip("golden_etf_next_month_bar_mode") else {
        return;
    };
    let panel = etf_panel();
...
fn golden_month_end_helper_and_warmup() {
    let Some(_) = real_or_skip("golden_month_end_helper_and_warmup") else {
        return;
    };
    let panel = etf_panel();
    let (_, rows) = load_shadow("shadow_S1_monthend_signals.csv");
...
fn golden_crypto_every_day_strict_gap_policy() {
    let Some(_) = real_or_skip("golden_crypto_every_day_strict_gap_policy") else {
        return;
    };
...
fn golden_crypto_every_day_reference_semantics() {
    let Some(_) = real_or_skip("golden_crypto_every_day_reference_semantics") else {
        return;
    };
