...
//!
//! Data: the ladder candles and the golden derived from them (`golden_fx_tsmom.csv`) are vendor-derived and are NOT in
//! this (public) repository, so the tests that need them are ENV-GATED: set `REFRULES_LADDER_DIR` to the local
//! `replication_ladder` folder (and `REFRULES_FX_GOLDEN_DIR` to the folder holding `golden_fx_tsmom.csv`, written by
//! `tests/data/gen_fx_golden.py`; defaults to `REFRULES_LADDER_DIR`). Without them those tests print `SKIPPED` and
//! pass. `golden_fx_weight_cap_binds_on_synthetic_panel` runs on the synthetic clip files that ship with the crate and
//! is always on.
...
/// The REAL FX golden (vendor-derived; env-gated, see the module docs).
fn load_golden(name: &str) -> Groups {
    let text = std::fs::read_to_string(real_fx_golden_path(name))
...
fn golden_fx_inputs_are_the_recorded_files() {
    // Always on: the synthetic clip files that ship with the crate.
    assert_eq!(sha256_of("fx_clip_candles.csv"), CLIP_CANDLES_SHA);
    assert_eq!(sha256_of("golden_fx_clip.csv"), CLIP_GOLDEN_SHA);
    // Env-gated: the vendor-derived real files.
    let Some(_) = real_or_skip("golden_fx_inputs_are_the_recorded_files (real files)") else {
        return;
    };
    assert_eq!(sha256_of_real("ladder_candles.csv"), LADDER_SHA);
    let golden = std::fs::read(real_fx_golden_path("golden_fx_tsmom.csv")).unwrap();
    assert_eq!(
        hex::encode(<sha2::Sha256 as sha2::Digest>::digest(&golden)),
        GOLDEN_SHA
    );
}
...
fn golden_fx_every_month_end_both_history_windows() {
    let Some(_) = real_or_skip("golden_fx_every_month_end_both_history_windows") else {
        return;
    };
...
fn golden_fx_coverage() {
    let Some(_) = real_or_skip("golden_fx_coverage") else {
        return;
    };
    let golden = load_golden("golden_fx_tsmom.csv");
...
fn golden_fx_strict_gap_policy_refuses_only_the_2019_hole() {
    let Some(_) = real_or_skip("golden_fx_strict_gap_policy_refuses_only_the_2019_hole") else {
        return;
    };
    let panel = ladder_fx_panel();
    let golden = load_golden("golden_fx_tsmom.csv");
...
fn golden_fx_next_month_bar_mode_agrees() {
    let Some(_) = real_or_skip("golden_fx_next_month_bar_mode_agrees") else {
        return;
    };
    let panel = ladder_fx_panel();
    let golden = load_golden("golden_fx_tsmom.csv");
...
fn golden_fx_ppy_window_quirk_is_real_and_reproduced() {
    let Some(_) = real_or_skip("golden_fx_ppy_window_quirk_is_real_and_reproduced") else {
        return;
    };
    let panel = ladder_fx_panel();
    let golden = load_golden("golden_fx_tsmom.csv");
...
fn golden_fx_history_start_before_the_file_changes_nothing() {
    let Some(_) = real_or_skip("golden_fx_history_start_before_the_file_changes_nothing") else {
        return;
    };
    let panel = ladder_fx_panel();
