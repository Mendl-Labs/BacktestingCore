...
MUTANTS = [
    ...
    # ---- DecisionPending, the live monthly cadence (council Ruling 1) -----------------------------------------------------------
    ("M63", "DecisionPending: the decision is the newest bar itself (no NextMonthBar completeness)", "src/schedule.rs",
     "Ok(bars.iter().rev().find(|b| !b.same_month(newest)).copied())", "Ok(Some(newest))"),
    ("M64", "DecisionPending: the month test of D_computable ignores the year", "src/schedule.rs",
     "Ok(bars.iter().rev().find(|b| !b.same_month(newest)).copied())",
     "Ok(bars.iter().rev().find(|b| b.month != newest.month).copied())"),
    ("M65", "DecisionPending: an already-acted decision is pending again (>= instead of >)", "src/schedule.rs",
     "Some(c) => last_acted.is_none_or(|a| c > a),", "Some(c) => last_acted.is_none_or(|a| c >= a),"),
    ("M66", "DecisionPending: the entry (nothing acted yet) is never pending", "src/schedule.rs",
     "Some(c) => last_acted.is_none_or(|a| c > a),", "Some(c) => last_acted.is_some_and(|a| c > a),"),
    ("M67", "DecisionPending: nothing computable counts as pending", "src/schedule.rs",
     "        None => false,\n        Some(c) => last_acted", "        None => true,\n        Some(c) => last_acted"),
    ("M68", "D_acted can move backwards (a replayed older decision overwrites it)", "src/schedule.rs",
     "Some(a) if a >= acted_on => a,", "Some(a) if a >= acted_on => acted_on,"),
    ("M69", "closed_bars includes the bar dated the run date (a forming bar)", "src/schedule.rs",
     "bars.partition_point(|b| *b < as_of)", "bars.partition_point(|b| *b <= as_of)"),
    ("M70", "due_on(DecisionPending) ignores the ledger", "src/schedule.rs",
     "Cadence::DecisionPending => decision_pending(i.computable_decision, i.last_acted),",
     "Cadence::DecisionPending => decision_pending(i.computable_decision, None),"),
    ("M71", "the stateless due() finds a computable decision (DecisionPending pending without a panel)", "src/schedule.rs",
     "DueInputs { date, next_bar, computable_decision: None, last_acted: None }",
     "DueInputs { date, next_bar, computable_decision: Some(date), last_acted: None }"),
    ("M72", "plan_flags_for ignores the cadences (every sleeve is daily)", "src/schedule.rs",
     ".map(|(c, i)| due_on(*c, i))", ".map(|(_, i)| due_on(Cadence::Daily, i))"),
    ("M73", "duplicate bar dates are accepted", "src/schedule.rs",
     "if bars[i] <= bars[i - 1] {", "if bars[i] < bars[i - 1] {"),
    ("M74", "unsorted bars are accepted", "src/schedule.rs",
     "if bars[i] <= bars[i - 1] {", "if false && bars[i] <= bars[i - 1] {"),
    ("M75", "evaluate_decision judges pending without the ledger", "src/schedule.rs",
     "pending: decision_pending(computable, last_acted) })", "pending: decision_pending(computable, None) })"),
    ("M76", "D_computable searches forward and returns the OLDEST bar outside the newest month", "src/schedule.rs",
     "bars.iter().rev().find(|b| !b.same_month(newest))", "bars.iter().find(|b| !b.same_month(newest))"),
    ("M77", "due_on(CalendarMonthEnd) answers like LastBarOfMonth (the documented wall-clock case is lost)", "src/schedule.rs",
     "Cadence::CalendarMonthEnd => i.date.is_calendar_month_end(),",
     "Cadence::CalendarMonthEnd => i.next_bar.is_none_or(|n| !i.date.same_month(n)),"),
    ("M78", "due_on(DecisionPending) is always due (the F1 every-day behaviour)", "src/schedule.rs",
     "Cadence::DecisionPending => decision_pending(i.computable_decision, i.last_acted),", "Cadence::DecisionPending => true,"),
    ("M79", "the stateless due() drops the next bar (LastBarOfMonth never sees the month change)", "src/schedule.rs",
     "due_on(cadence, &DueInputs::stateless(date, next_bar))", "due_on(cadence, &DueInputs::stateless(date, None))"),
    ("M80", "plan_flags_for ignores the mode (always only-due)", "src/schedule.rs",
     "plan_flags(mode, &dues, tradable)", "plan_flags(BookCadence::PerSleeve, &dues, tradable)"),
]
